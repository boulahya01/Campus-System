/* TypeScript Axios client for Campus System API (minimal wrapper)
...
This file is intentionally small — expand per-frontend needs.
*/
...
export default class ApiClient {
  ...
  // Materials: upload (multipart) — using fetch for stream-friendly upload
  async uploadMaterial(moduleId: number, file: File, title?: string) {
    const form = new FormData()
    form.append('file', file)
    if (title) form.append('title', title)

    // include Authorization header from axios instance
  ...
  // downloadMaterial returns either a presigned URL or blob response
  async downloadMaterial(materialId: number) {
    const r = await this.api.get(`/api/materials/${materialId}/download`)
    // server returns {url: <presigned>} or serves file directly
    const body = r.data
    if (body && body.url) return body.url
    // otherwise fetch file as blob
